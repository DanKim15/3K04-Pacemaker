...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(pacemaker_simulink_old_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    pacemaker_simulink_old_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(pacemaker_simulink_old_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pacemaker_simulink_old_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  pacemaker_simulink_old_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(pacemaker_simulink_old_M, 0);

  /* Initialize model */
  pacemaker_simulink_old_initialize();
