...
#define pacemaker_simulin_IN_EGRAM_STOP ((uint8_T)3U)
#define pacemaker_simulink_IN_ECHO     ((uint8_T)1U)
#define pacemaker_simulink_IN_EGRAM_REQ ((uint8_T)2U)
#define pacemaker_simulink_IN_INIT     ((uint8_T)4U)
#define pacemaker_simulink_IN_SET      ((uint8_T)5U)
#define pacemaker_simulink_IN_STANDBY  ((uint8_T)6U)
...
static void p_SystemCore_rele_l(const freedomk64f_SCIRead_pacemaker_T *obj);
static void p_SystemCore_dele_l(const freedomk64f_SCIRead_pacemaker_T *obj);
static void pacemaker_sim_matlabCodegenHa_l(freedomk64f_SCIRead_pacemaker_T *obj);
static void p_SystemCore_rele_e(const freedomk64f_AnalogInput_pacem_T *obj);
static void p_SystemCore_dele_e(const freedomk64f_AnalogInput_pacem_T *obj);
static void pacemaker_sim_matlabCodegenHa_e(freedomk64f_AnalogInput_pacem_T *obj);
...
static void p_SystemCore_rele_h(const freedomk64f_SCIWrite_pacemake_T *obj);
static void p_SystemCore_dele_h(const freedomk64f_SCIWrite_pacemake_T *obj);
static void pacemaker_sim_matlabCodegenHa_h(freedomk64f_SCIWrite_pacemake_T *obj);
...
static void pacemaker_s_SystemCore_setup_o3(freedomk64f_SCIRead_pacemaker_T *obj);
static void pacemaker_si_SystemCore_setup_o(freedomk64f_SCIWrite_pacemake_T *obj);
...
void pacemaker_simulink_SetEventsForThisBaseStep(boolean_T *eventFlags)
{
  ...
  eventFlags[2] = ((boolean_T)rtmStepTask(pacemaker_simulink_M, 2));
}
...
static void rate_monotonic_scheduler(void)
{
  ...
  pacemaker_simulink_M->Timing.RateInteraction.TID0_2 =
    (pacemaker_simulink_M->Timing.TaskCounters.TID[2] == 0);
  ...
  if ((pacemaker_simulink_M->Timing.TaskCounters.TID[1]) > 4) {/* Sample time: [0.005s, 0.0s] */
    pacemaker_simulink_M->Timing.TaskCounters.TID[1] = 0;
  }

  (pacemaker_simulink_M->Timing.TaskCounters.TID[2])++;
  if ((pacemaker_simulink_M->Timing.TaskCounters.TID[2]) > 19) {/* Sample time: [0.02s, 0.0s] */
    pacemaker_simulink_M->Timing.TaskCounters.TID[2] = 0;
  }
}
...
static void pacemaker_s_SystemCore_setup_o3(freedomk64f_SCIRead_pacemaker_T *obj)
{
  ...
  obj->isInitialized = 1;
  pacemaker_simulink_B.TxPinLoc = MW_UNDEFINED_VALUE;
  pacemaker_simulink_B.SCIModuleLoc_m = 0;
  obj->MW_SCIHANDLE = MW_SCI_Open(&pacemaker_simulink_B.SCIModuleLoc_m, false,
    10U, pacemaker_simulink_B.TxPinLoc);
...
static void pacemaker_si_SystemCore_setup_o(freedomk64f_SCIWrite_pacemake_T *obj)
{
  ...
  obj->isSetupComplete = false;
  obj->isInitialized = 1;
  pacemaker_simulink_B.RxPinLoc = MW_UNDEFINED_VALUE;
  pacemaker_simulink_B.SCIModuleLoc = 0;
  obj->MW_SCIHANDLE = MW_SCI_Open(&pacemaker_simulink_B.SCIModuleLoc, false,
    pacemaker_simulink_B.RxPinLoc, 10U);
  MW_SCI_SetBaudrate(obj->MW_SCIHANDLE, 115200U);
  StopBitsValue = MW_SCI_STOPBITS_1;
  ParityValue = MW_SCI_PARITY_NONE;
  MW_SCI_SetFrameFormat(obj->MW_SCIHANDLE, 8, ParityValue, StopBitsValue);
...
void pacemaker_simulink_step0(void)    /* Sample time: [0.001s, 0.0s] */
{
  ...
  if (pacemaker_simulink_DW.obj_e.SampleTime !=
      pacemaker_simulink_P.SerialReceive_SampleTime) {
    pacemaker_simulink_DW.obj_e.SampleTime =
      pacemaker_simulink_P.SerialReceive_SampleTime;
  }

  status = MW_SCI_Receive(pacemaker_simulink_DW.obj_e.MW_SCIHANDLE,
    RxDataLocChar, 20U);
  ...
  if (pacemaker_simulink_DW.is_active_c1_pacemaker_simulink == 0U) {
    ...
    pacemaker_simulink_B.EGRAM_ENABLE_out = false;
  } else {
    switch (pacemaker_simulink_DW.is_c1_pacemaker_simulink) {
     ...
     case pacemaker_simulink_IN_EGRAM_REQ:
      pacemaker_simulink_DW.is_c1_pacemaker_simulink =
        pacemaker_simulink_IN_STANDBY;
      rtb_echo_request = false;
      break;

     case pacemaker_simulin_IN_EGRAM_STOP:
      pacemaker_simulink_DW.is_c1_pacemaker_simulink =
        pacemaker_simulink_IN_STANDBY;
      rtb_echo_request = false;
      break;
     ...
     default:
      ...
      if (status == 0) {
        if (pacemaker_simulink_B.RxData[0] == 22) {
          ...
          } else if (pacemaker_simulink_B.RxData[1] == 3) {
            pacemaker_simulink_DW.is_c1_pacemaker_simulink =
              pacemaker_simulink_IN_EGRAM_REQ;
            pacemaker_simulink_B.EGRAM_ENABLE_out = true;
          } else if (pacemaker_simulink_B.RxData[1] == 5) {
            pacemaker_simulink_DW.is_c1_pacemaker_simulink =
              pacemaker_simulin_IN_EGRAM_STOP;
            pacemaker_simulink_B.EGRAM_ENABLE_out = false;
          } else {
            pacemaker_simulink_DW.is_c1_pacemaker_simulink =
              pacemaker_simulink_IN_STANDBY;
   ...
   */
  if (rtb_echo_request) {
    pacemaker_simulink_DW.UART_Tx_MODE = true;

    /* DataTypeConversion: '<S17>/Data Type Conversion1' incorporates:
     *  DataStoreWrite: '<S7>/Data Store Write1'
     */
    pacemaker_simulink_B.DataTypeConversion1_l =
      pacemaker_simulink_B.HYSTERESIS_out;

    /* S-Function (any2byte): '<S17>/Byte Pack' incorporates:
     *  Constant: '<S17>/MSGTYPE_TX'
     *  Constant: '<S17>/SYNC_TX'
     */

    /* Pack: <S17>/Byte Pack */
    (void) memcpy(&pacemaker_simulink_B.BytePack[0],
                  &pacemaker_simulink_P.SYNC_TX_Value_c,
                  1);
    (void) memcpy(&pacemaker_simulink_B.BytePack[1],
                  &pacemaker_simulink_P.MSGTYPE_TX_Value_c,
                  1);
    ...
    /* SignalConversion generated from: '<S17>/EchoValid' incorporates:
     *  Constant: '<S17>/Constant'
     */
    pacemaker_simulink_B.OutportBufferForEchoValid =
      pacemaker_simulink_P.Constant_Value_e;
  } else {
    if (pacemaker_simulink_DW.UART_Tx_MODE) {
      /* Disable for Outport: '<S17>/EchoPacket' */
      for (i = 0; i < 20; i++) {
        pacemaker_simulink_B.BytePack[i] = pacemaker_simulink_P.EchoPacket_Y0;
      }

      /* End of Disable for Outport: '<S17>/EchoPacket' */

      /* Disable for Outport: '<S17>/EchoValid' */
      pacemaker_simulink_B.OutportBufferForEchoValid =
        pacemaker_simulink_P.EchoValid_Y0;
      pacemaker_simulink_DW.UART_Tx_MODE = false;
    }
  }

  /* End of Outputs for SubSystem: '<S7>/UART_Tx' */

  /* Outputs for Enabled SubSystem: '<S7>/UART_EGRAM_Tx' incorporates:
   *  EnablePort: '<S14>/Enable'
   */
  if (pacemaker_simulink_B.EGRAM_ENABLE_out) {
    pacemaker_simulink_DW.UART_EGRAM_Tx_MODE = true;

    /* MATLABSystem: '<S14>/Analog Input' */
    if (pacemaker_simulink_DW.obj_i2.SampleTime !=
        pacemaker_simulink_P.AnalogInput_SampleTime) {
      pacemaker_simulink_DW.obj_i2.SampleTime =
        pacemaker_simulink_P.AnalogInput_SampleTime;
    }

    MW_AnalogIn_Start(pacemaker_simulink_DW.obj_i2.MW_ANALOGIN_HANDLE);
    MW_AnalogInSingle_ReadResult(pacemaker_simulink_DW.obj_i2.MW_ANALOGIN_HANDLE,
      &rtb_AnalogInput_0, 7);

    /* MATLABSystem: '<S14>/Analog Input1' */
    if (pacemaker_simulink_DW.obj_p.SampleTime !=
        pacemaker_simulink_P.AnalogInput1_SampleTime) {
      pacemaker_simulink_DW.obj_p.SampleTime =
        pacemaker_simulink_P.AnalogInput1_SampleTime;
    }

    MW_AnalogIn_Start(pacemaker_simulink_DW.obj_p.MW_ANALOGIN_HANDLE);
    MW_AnalogInSingle_ReadResult(pacemaker_simulink_DW.obj_p.MW_ANALOGIN_HANDLE,
      &rtb_Add, 7);
    ...
    if (rtIsNaN(rtb_AnalogInput_0) || rtIsInf(rtb_AnalogInput_0)) {
      ...
      rtb_AnalogInput_0 = fmod(rtb_AnalogInput_0, 65536.0);
    }

    pacemaker_simulink_B.DataTypeConversion = (uint16_T)(rtb_AnalogInput_0 < 0.0
      ? (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_AnalogInput_0 : (int32_T)
     ...
     *  MATLABSystem: '<S14>/Analog Input1'
     */
    rtb_AnalogInput_0 = floor(rtb_Add);
    if (rtIsNaN(rtb_AnalogInput_0) || rtIsInf(rtb_AnalogInput_0)) {
      rtb_AnalogInput_0 = 0.0;
    } else {
      rtb_AnalogInput_0 = fmod(rtb_AnalogInput_0, 65536.0);
    ...
    pacemaker_simulink_B.DataTypeConversion1 = (uint16_T)(rtb_AnalogInput_0 <
      0.0 ? (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_AnalogInput_0 : (int32_T)
      (uint16_T)rtb_AnalogInput_0);
     ...
     */
    rtb_AnalogInput_0 = floor(pacemaker_simulink_P.Constant_Value);
    if (rtIsNaN(rtb_AnalogInput_0) || rtIsInf(rtb_AnalogInput_0)) {
      rtb_Add = 0.0;
    } else {
      rtb_Add = fmod(rtb_AnalogInput_0, 65536.0);
    }

    pacemaker_simulink_B.DataTypeConversion2 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);
    ...
    pacemaker_simulink_B.DataTypeConversion3 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);
    ...
    pacemaker_simulink_B.DataTypeConversion4 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);
    ...
    pacemaker_simulink_B.DataTypeConversion5 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);
    ...
    pacemaker_simulink_B.DataTypeConversion6 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);
    ...
    pacemaker_simulink_B.DataTypeConversion7 = (uint16_T)(rtb_Add < 0.0 ?
      (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_Add : (int32_T)(uint16_T)
      rtb_Add);

    /* DataTypeConversion: '<S14>/Data Type Conversion8' */
    pacemaker_simulink_B.DataTypeConversion8 = (uint16_T)(rtb_AnalogInput_0 <
      0.0 ? (int32_T)(uint16_T)-(int16_T)(uint16_T)-rtb_AnalogInput_0 : (int32_T)
      (uint16_T)rtb_AnalogInput_0);

    /* S-Function (any2byte): '<S14>/Byte Pack' incorporates:
     *  Constant: '<S14>/MSGTYPE_TX'
     *  Constant: '<S14>/SYNC_TX'
     */

    /* Pack: <S14>/Byte Pack */
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[0],
                  &pacemaker_simulink_P.SYNC_TX_Value,
                  1);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[1],
                  &pacemaker_simulink_P.MSGTYPE_TX_Value,
                  1);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[2],
                  &pacemaker_simulink_B.DataTypeConversion,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[4],
                  &pacemaker_simulink_B.DataTypeConversion1,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[6],
                  &pacemaker_simulink_B.DataTypeConversion2,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[8],
                  &pacemaker_simulink_B.DataTypeConversion3,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[10],
                  &pacemaker_simulink_B.DataTypeConversion4,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[12],
                  &pacemaker_simulink_B.DataTypeConversion5,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[14],
                  &pacemaker_simulink_B.DataTypeConversion6,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[16],
                  &pacemaker_simulink_B.DataTypeConversion7,
                  2);
    (void) memcpy(&pacemaker_simulink_B.BytePack_i[18],
                  &pacemaker_simulink_B.DataTypeConversion8,
                  2);

    /* SignalConversion generated from: '<S14>/EgramValid' incorporates:
     *  Constant: '<S14>/Constant1'
     */
    pacemaker_simulink_B.OutportBufferForEgramValid =
      pacemaker_simulink_P.Constant1_Value_a;
  } else {
    if (pacemaker_simulink_DW.UART_EGRAM_Tx_MODE) {
      /* Disable for Outport: '<S14>/EgramPacket' */
      for (i = 0; i < 20; i++) {
        pacemaker_simulink_B.BytePack_i[i] = pacemaker_simulink_P.EgramPacket_Y0;
      }

      /* End of Disable for Outport: '<S14>/EgramPacket' */

      /* Disable for Outport: '<S14>/EgramValid' */
      pacemaker_simulink_B.OutportBufferForEgramValid =
        pacemaker_simulink_P.EgramValid_Y0;
      pacemaker_simulink_DW.UART_EGRAM_Tx_MODE = false;
    }
  }

  /* End of Outputs for SubSystem: '<S7>/UART_EGRAM_Tx' */
   ...
   */
  for (i = 0; i < 20; i++) {
    if (pacemaker_simulink_B.OutportBufferForEchoValid) {
      RxDataLocChar[i] = pacemaker_simulink_B.BytePack[i];
    } else if (pacemaker_simulink_B.OutportBufferForEgramValid) {
      /* Switch: '<S16>/Switch' */
      RxDataLocChar[i] = pacemaker_simulink_B.BytePack_i[i];
    } else {
      RxDataLocChar[i] = pacemaker_simulink_P.Constant_Value_g[i];
    }
  }
  ...
  if (pacemaker_simulink_M->Timing.RateInteraction.TID0_1) {
    for (i = 0; i < 20; i++) {
      pacemaker_simulink_DW.RateTransition_Buffer[i] = RxDataLocChar[i];
    }
  }
   ...
   *  Product: '<S2>/Divide2'
   */
  rtb_AnalogInput_0 = floor(pacemaker_simulink_P.Constant_Value_l / (real_T)
    pacemaker_simulink_B.LOWER_RATE_LIMIT_out);
  if (rtIsNaN(rtb_AnalogInput_0) || rtIsInf(rtb_AnalogInput_0)) {
    rtb_AnalogInput_0 = 0.0;
  } else {
  ...
  Cast2 = (uint16_T)(rtb_AnalogInput_0 < 0.0 ? (int32_T)(uint16_T)-(int16_T)
                     (uint16_T)-rtb_AnalogInput_0 : (int32_T)(uint16_T)
                     rtb_AnalogInput_0);

  /* End of DataTypeConversion: '<S2>/Cast2' */

  /* RateTransition: '<Root>/Rate Transition' */
  if (pacemaker_simulink_M->Timing.RateInteraction.TID0_2) {
   ...
   *  Sum: '<S13>/Subtract1'
   */
  rtb_AnalogInput_0 = pacemaker_simulink_P.MAX_SENSOR_RATE_Value - (real_T)
    pacemaker_simulink_B.LOWER_RATE_LIMIT_out;
   ...
   *  Sum: '<S9>/Subtract1'
   */
  rtb_Add = rtb_AnalogInput_0 / pacemaker_simulink_P.ResponseFactors_Value *
  ...
  if (pacemaker_simulink_B.RateTransition -
     ...
     */
    pacemaker_simulink_DW.PreviousRate_DSTATE += rtb_AnalogInput_0 /
   ...
   *  UnitDelay: '<S2>/Previous Rate'
   */
  rtb_AnalogInput_0 = floor(pacemaker_simulink_P.Constant_Value_l /
    pacemaker_simulink_DW.PreviousRate_DSTATE);
  if (rtIsNaN(rtb_AnalogInput_0) || rtIsInf(rtb_AnalogInput_0)) {
    rtb_AnalogInput_0 = 0.0;
  } else {
  ...
  Cast1 = (uint16_T)(rtb_AnalogInput_0 < 0.0 ? (int32_T)(uint16_T)-(int16_T)
                     (uint16_T)-rtb_AnalogInput_0 : (int32_T)(uint16_T)
                     rtb_AnalogInput_0);
  ...
  MW_digitalIO_write(pacemaker_simulink_DW.obj_ep.MW_DIGITALIO_HANDLE,
                     pacemaker_simulink_B.ATR_PACE_CTRL != 0.0);

  /* MATLABSystem: '<S3>/Digital Write4' */
  MW_digitalIO_write(pacemaker_simulink_DW.obj_m.MW_DIGITALIO_HANDLE,
                     pacemaker_simulink_B.VENT_PACE_CTRL != 0.0);
  ...
  MW_digitalIO_write(pacemaker_simulink_DW.obj_pv.MW_DIGITALIO_HANDLE,
                     pacemaker_simulink_B.VENT_GND_CTRL != 0.0);
...
void pacemaker_simulink_step1(void)    /* Sample time: [0.005s, 0.0s] */
{
  uint8_T status;
  uint8_T TxDataLocChar[20];
  ...
  while (status != 0) {
    memcpy((void *)&TxDataLocChar[0], (void *)
           &pacemaker_simulink_DW.RateTransition_Buffer[0], (uint32_T)((size_t)
            20 * sizeof(uint8_T)));
    status = MW_SCI_Transmit(pacemaker_simulink_DW.obj_mi.MW_SCIHANDLE,
      TxDataLocChar, 20U);
  }
...
void pacemaker_simulink_step2(void)    /* Sample time: [0.02s, 0.0s] */
{
  int16_T b_output[3];
  uint8_T status;
  ...
  rtb_Clamp01 = fabs((((((((rtb_FXOS87006AxesSensor_idx_0 +
    ...
    pacemaker_simulink_DW.UnitDelay3_DSTATE) +
                     ...
                     pacemaker_simulink_P.Constant_Value_li -
...
void pacemaker_simulink_initialize(void)
{
...

  {
    ...
    freedomk64f_AnalogInput_pacem_T *obj_3;
    int32_T i;
    ...
    /* SystemInitialize for Enabled SubSystem: '<S7>/UART_EGRAM_Tx' */
    /* Start for MATLABSystem: '<S14>/Analog Input' */
    pacemaker_simulink_DW.obj_i2.matlabCodegenIsDeleted = true;
    pacemaker_simulink_DW.obj_i2.isInitialized = 0;
    pacemaker_simulink_DW.obj_i2.SampleTime = -1.0;
    pacemaker_simulink_DW.obj_i2.matlabCodegenIsDeleted = false;
    pacemaker_simulink_DW.obj_i2.SampleTime =
      pacemaker_simulink_P.AnalogInput_SampleTime;
    obj_3 = &pacemaker_simulink_DW.obj_i2;
    pacemaker_simulink_DW.obj_i2.isSetupComplete = false;
    pacemaker_simulink_DW.obj_i2.isInitialized = 1;
    obj_3->MW_ANALOGIN_HANDLE = MW_AnalogInSingle_Open(16U);
    pacemaker_simulink_B.trigger_val = MW_ANALOGIN_SOFTWARE_TRIGGER;
    MW_AnalogIn_SetTriggerSource(pacemaker_simulink_DW.obj_i2.MW_ANALOGIN_HANDLE,
      pacemaker_simulink_B.trigger_val, 0U);
    pacemaker_simulink_DW.obj_i2.isSetupComplete = true;
    ...
    pacemaker_simulink_DW.obj_p.SampleTime = -1.0;
    pacemaker_simulink_DW.obj_p.matlabCodegenIsDeleted = false;
    pacemaker_simulink_DW.obj_p.SampleTime =
      pacemaker_simulink_P.AnalogInput1_SampleTime;
    obj_3 = &pacemaker_simulink_DW.obj_p;
    pacemaker_simulink_DW.obj_p.isSetupComplete = false;
    pacemaker_simulink_DW.obj_p.isInitialized = 1;
    obj_3->MW_ANALOGIN_HANDLE = MW_AnalogInSingle_Open(17U);
    pacemaker_simulink_B.trigger_val = MW_ANALOGIN_SOFTWARE_TRIGGER;
    MW_AnalogIn_SetTriggerSource(pacemaker_simulink_DW.obj_p.MW_ANALOGIN_HANDLE,
      pacemaker_simulink_B.trigger_val, 0U);
    pacemaker_simulink_DW.obj_p.isSetupComplete = true;

    /* SystemInitialize for Outport: '<S14>/EgramValid' */
    pacemaker_simulink_B.OutportBufferForEgramValid =
      pacemaker_simulink_P.EgramValid_Y0;

    /* SystemInitialize for Enabled SubSystem: '<S7>/UART_Tx' */
    for (i = 0; i < 20; i++) {
      /* SystemInitialize for Outport: '<S14>/EgramPacket' */
      pacemaker_simulink_B.BytePack_i[i] = pacemaker_simulink_P.EgramPacket_Y0;

      /* SystemInitialize for Outport: '<S17>/EchoPacket' */
      pacemaker_simulink_B.BytePack[i] = pacemaker_simulink_P.EchoPacket_Y0;
    }

    /* End of SystemInitialize for SubSystem: '<S7>/UART_EGRAM_Tx' */

    /* SystemInitialize for Outport: '<S17>/EchoValid' */
    pacemaker_simulink_B.OutportBufferForEchoValid =
      pacemaker_simulink_P.EchoValid_Y0;
    ...
    pacemaker_simulink_DW.obj_e.SampleTime =
      pacemaker_simulink_P.SerialReceive_SampleTime;
    pacemaker_s_SystemCore_setup_o3(&pacemaker_simulink_DW.obj_e);
    ...
    pacemaker_simulink_DW.obj_ep.matlabCodegenIsDeleted = true;
    pacemaker_simulink_DW.obj_ep.isInitialized = 0;
    pacemaker_simulink_DW.obj_ep.matlabCodegenIsDeleted = false;
    obj_0 = &pacemaker_simulink_DW.obj_ep;
    pacemaker_simulink_DW.obj_ep.isSetupComplete = false;
    pacemaker_simulink_DW.obj_ep.isInitialized = 1;
    obj_0->MW_DIGITALIO_HANDLE = MW_digitalIO_open(8U, 1);
    pacemaker_simulink_DW.obj_ep.isSetupComplete = true;

    /* Start for MATLABSystem: '<S3>/Digital Write4' */
    pacemaker_simulink_DW.obj_m.matlabCodegenIsDeleted = true;
    pacemaker_simulink_DW.obj_m.isInitialized = 0;
    pacemaker_simulink_DW.obj_m.matlabCodegenIsDeleted = false;
    obj_0 = &pacemaker_simulink_DW.obj_m;
    pacemaker_simulink_DW.obj_m.isSetupComplete = false;
    pacemaker_simulink_DW.obj_m.isInitialized = 1;
    obj_0->MW_DIGITALIO_HANDLE = MW_digitalIO_open(9U, 1);
    pacemaker_simulink_DW.obj_m.isSetupComplete = true;
    ...
    pacemaker_simulink_DW.obj_pv.matlabCodegenIsDeleted = true;
    pacemaker_simulink_DW.obj_pv.isInitialized = 0;
    pacemaker_simulink_DW.obj_pv.matlabCodegenIsDeleted = false;
    obj_0 = &pacemaker_simulink_DW.obj_pv;
    pacemaker_simulink_DW.obj_pv.isSetupComplete = false;
    pacemaker_simulink_DW.obj_pv.isInitialized = 1;
    obj_0->MW_DIGITALIO_HANDLE = MW_digitalIO_open(12U, 1);
    pacemaker_simulink_DW.obj_pv.isSetupComplete = true;
    ...
    pacemaker_simulink_DW.obj_mi.isInitialized = 0;
    pacemaker_simulink_DW.obj_mi.matlabCodegenIsDeleted = false;
    pacemaker_si_SystemCore_setup_o(&pacemaker_simulink_DW.obj_mi);
...
void pacemaker_simulink_terminate(void)
{
  ...
  pacemaker_sim_matlabCodegenHa_l(&pacemaker_simulink_DW.obj_e);

  /* Terminate for Enabled SubSystem: '<S7>/UART_EGRAM_Tx' */
  /* Terminate for MATLABSystem: '<S14>/Analog Input' */
  pacemaker_sim_matlabCodegenHa_e(&pacemaker_simulink_DW.obj_i2);

  /* Terminate for MATLABSystem: '<S14>/Analog Input1' */
  pacemaker_sim_matlabCodegenHa_e(&pacemaker_simulink_DW.obj_p);

  /* End of Terminate for SubSystem: '<S7>/UART_EGRAM_Tx' */
  ...
  matlabCodegenHandle_matlabCo_o3(&pacemaker_simulink_DW.obj_ep);

  /* Terminate for MATLABSystem: '<S3>/Digital Write4' */
  matlabCodegenHandle_matlabCo_o3(&pacemaker_simulink_DW.obj_m);
  ...
  matlabCodegenHandle_matlabCo_o3(&pacemaker_simulink_DW.obj_pv);
  ...
  pacemaker_sim_matlabCodegenHa_h(&pacemaker_simulink_DW.obj_mi);
