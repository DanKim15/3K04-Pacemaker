...
mw_signal_event_t subrateTaskSem[2];
int taskId[2];
...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    for (i = 1
         ; i <= 2; i++) {
      if (rtmStepTask(pacemaker_simulink_M, i)
          ) {
        mw_osSignalEventRelease(&subrateTaskSem[ i - 1
          ]);
      }
    }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pacemaker_simulink_M, "stopping the model");
  runModel = 0;
}
