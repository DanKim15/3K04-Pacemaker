...
class SerialManager:
    ...
    def _encode_params(self, params: Dict[str, Any]) -> bytes:
        ...
        Layout:
          ...
          2: MODE_SELECT (0..7)
          3: HYSTERESIS (0/1)
          4-5: LRL (uint16, bpm, little-endian)
          6-7: URL (uint16, bpm, little-endian)
          8-9:  Atrial amplitude register (uint16, little-endian)
         10-11: Ventricular amplitude register (uint16, little-endian)
         12-13: Atrial pulse width (ms, uint16, little-endian)
         14-15: Ventricular pulse width (ms, uint16, little-endian)
         16-17: ARP (ms, uint16, little-endian)
         18-19: VRP (ms, uint16, little-endian)
        """
        buf = bytearray(20)
        buf[0] = SYNC_BYTE
